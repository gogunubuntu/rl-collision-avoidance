...
(added by Hyunjin Lee)modyfied code for dqn algorithm
...
```
(added by Hyunjin Lee)To train Stage1 using DQN, modify the hyper-parameters in `dqn_stage1.py` as you like, and running the following command:  
q - network's input : laser observation, robot position, goal point, speed(linear velocity and angular velocity)  
q - network's output : action value of 30 angular velocities in range from -1 to +1
```
rosrun stage_ros_add_pose_and_crash stageros -g worlds/stage1.world
